...
## Official Harness Docs (Upstream References)

Each vendor's spec evolves — when adding or fixing sync behavior for a harness, check the current upstream docs before changing `.ai/scripts/sync.js` or the target paths.

| Harness | Instructions / Memory | Skills | MCP |
|---|---|---|---|
| Claude Code | https://code.claude.com/docs/en/memory | https://code.claude.com/docs/en/skills | https://code.claude.com/docs/en/mcp |
| OpenAI Codex | https://developers.openai.com/codex/guides/agents-md | https://developers.openai.com/codex/skills | https://developers.openai.com/codex/mcp |
| GitHub Copilot | https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/add-custom-instructions | https://docs.github.com/en/copilot/concepts/agents/about-agent-skills | https://docs.github.com/en/copilot/concepts/context/mcp |
| Gemini CLI | https://geminicli.com/docs/cli/gemini-md/ | https://geminicli.com/docs/cli/using-agent-skills/ + https://geminicli.com/docs/cli/custom-commands/ | https://geminicli.com/docs/tools/mcp-server/ |

> **Important — if this repo uses the `.ai/` sync harness:** the root agent files (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.agents/context/AGENTS.md`) are **generated** — composed by `.ai/scripts/sync.js` from `.ai/project.md` (project-specific) + `.ai/base.md` (portable). Never edit the generated files directly; any "edit the root agent doc" instruction below means *edit the `.ai/` sources and run `node .ai/scripts/sync.js`*. You can tell a file is generated by the `<!-- ai-sync-generated -->` marker at the end. The instructions below apply as written only in a vanilla environment without the sync harness.
...
In a sync-harness repo, this content lives in `.ai/project.md` + `.ai/base.md` and is composed into the root files automatically.
...
3. Rewrite root agent doc into a TOC-style map (sync-harness repos: rewrite `.ai/project.md`/`.ai/base.md` and re-run the sync instead).
...
- Root: `AGENTS.md`/`CLAUDE.md` (short map only)
  - Again, this content lives in `.ai/project.md` + `.ai/base.md` in a sync-harness repo, and is composed into the root files automatically.
...
- If the repo contains `.ai/scripts/sync.js`, the root agent files are generated:
  edit `.ai/project.md` + `.ai/base.md` instead and run `node .ai/scripts/sync.js`.
